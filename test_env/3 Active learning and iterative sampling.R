...
other_features <- c("year","length","sentiment","keyword_count")
...
# Prepare to run RFs
upto <- c(seq(from=30, to=nrow(training), by=3), nrow(training))
upto <- unique(upto)
n_upto <- length(upto)
...
results <- foreach(i=1:n_upto, .packages=c('randomForest','mltest')) %dopar%{
  ...
  summ_manifesto <- plyr::ddply(.data=preds, .variables="manifesto", .fun=function(x) get_summ(x=x))
...
res_manifesto <- plyr::ldply(.data=results, .fun=function(x) x$summ_manifesto)
...
## Errors
res_errors <- plyr::ldply(.data=results, .fun=function(x) x$oob)
...
for(i in 1:8)
{
  ...
  coverage_sample <- rbind(coverage_sample, tmp[,c("id","text")])
}
...
new_sample <- new_sample[duplicated(new_sample$id) == FALSE,]
new_sample <- new_sample[sample(1:nrow(new_sample)),]
write.csv(new_sample,file=new_sample_name)
